...
## Dashboard (webpage with charts)

A dashboard lives at `docs/index.html` and shows: total scans, last scan
time, best spread ever seen, a chart of the best spread over time, and
tables of recent scans and opportunities. It reads directly from
`scan_log.csv` / `opportunities.csv` in your repo — no server needed.
...
4. GitHub will give you a URL like
   `https://ribbon-ops.github.io/Bot/` — open it any time.
   It can take a minute or two to go live the first time.

The dashboard auto-refreshes every 60 seconds while open, and has a manual
"Refresh now" button too. If it shows "No scan data found yet," the
workflow hasn't run/committed data yet — check the Actions tab.

If you ever rename the repo or move it to a different account, open
`docs/index.html`, find the `OWNER` and `REPO` constants near the top of
the `<script>` block, and update them to match.
