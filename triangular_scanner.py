...
# Note: we deliberately try data-api.binance.vision first, not api.binance.com.
...
# are. data-api.binance.vision is Binance's own dedicated public market-data
# endpoint that serves the same data without that regional restriction.
...
def fetch_order_books():
    ...
    Tries data-api.binance.vision first (works from GitHub Actions' US-hosted
    runners); falls back to the other mirrors if that ever fails, so a single
    endpoint hiccup doesn't kill the whole run.
    """
    last_error = None
