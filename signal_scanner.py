...
CONF = {
 "EURUSD":{"pip":0.0001,"tp":0.3,"sl":1.0,"sweep":0.15},
 "GBPUSD":{"pip":0.0001,"tp":0.3,"sl":1.0,"sweep":0.15},
 "USDJPY":{"pip":0.01,"tp":0.3,"sl":1.0,"sweep":0.15},
 "XAUUSD":{"pip":0.1,"tp":0.4,"sl":1.5,"sweep":0.2},
}
...
def main():
        ...
        try:
            ...
            for tr in opens[sym]:
                ...
                if pips>=cfg["sweep"]:
                    ...
                    print(f"CLOSE WIN {sym} {pips:.2f}p ${pips*0.01:.4f}")
                elif pips<=-cfg["sl"]:
                    ex=os.path.exists("closed_trades.csv")
                    with open("closed_trades.csv","a",newline="") as f:
                        w=csv.DictWriter(f,fieldnames=["timestamp_utc","symbol","direction","entry_price","exit_price","outcome","pips","opened_at"])
                    ...
                    print(f"CLOSE LOSS {sym} {pips:.2f}p")
                else:
                    tr["current_price"]=cur
                    if (is_buy and cur>=tr["take_profit"]) or (not is_buy and cur<=tr["take_profit"]):
                        ex=os.path.exists("closed_trades.csv")
                        with open("closed_trades.csv","a",newline="") as f:
                            w=csv.DictWriter(f,fieldnames=["timestamp_utc","symbol","direction","entry_price","exit_price","outcome","pips","opened_at"])
                            if not ex: w.writeheader()
    ...
    os.makedirs("Bot/Bot",exist_ok=True)
    for fn in ["open_signals.json","closed_trades.csv","signals_log.csv"]:
        if os.path.exists(fn):
            try:
                import shutil
                shutil.copy(fn,f"Bot/{fn}")
                shutil.copy(fn,f"Bot/Bot/{fn}")
            except: pass
